  ...
  <>
    <button
      ...
      className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-[13px] font-medium transition-colors duration-150 text-left
        ${danger ? 'text-red-600 dark:text-red-400 hover:bg-red-500/10' : 'text-text-primary hover:bg-surface-hover'}`}
    >
      <span className={danger ? 'text-red-600 dark:text-red-400' : 'text-text-muted'}>{icon}</span>
      {label}
    </button>
    {dividerAfter && <div className='h-px bg-border my-1 mx-1' />}
  </>
);
...
const ChatOptionsMenu = ({ chatId, anchorEl, onClose, onDelete, onRename }) => {
  ...
  const menu = (
    <div
      ...
      className='py-1.5 px-1.5 bg-bg-card border border-border rounded-xl shadow-xl dark:shadow-2xl dark:shadow-black/50 backdrop-blur-sm'
